...
import type { DisplayPortfolioAvg } from "@/lib/queries/display";
import {
  formatNumber,
  formatPercentage,
  formatCurrency,
...
export interface DisplayLeaderboardProps {
  tieredCreatives: TieredCreative[];
  portfolioAvg: DisplayPortfolioAvg;
  kpiType: PrimaryKpi;
}
...
/** Ad type badge with colour coding per design system */
function AdTypeBadge({ adType }: { adType: string }) {
  // Colour mapping: Responsive Display = blue, Image Ad = grey, Discovery = amber
  const colourMap: Record<string, string> = {
    "Responsive Display": "bg-[#1A73E8]/10 text-[#1A73E8]",
    "Image Ad": "bg-[#9AA0A6]/10 text-[#6B7280]",
    "Discovery Multi-Asset": "bg-[#FBBC04]/10 text-[#9A7400]",
  };

  const style = colourMap[adType] ?? "bg-gray-100 text-gray-600";
...
/** Get display name for a creative */
function creativeName(creative: TieredCreative): string {
  const adName = String(creative.adName ?? "");
  if (adName) return adName;
  return `Ad ${creative.adId}`;
}

/**
 * Top and bottom creative leaderboard tables for Display ads.
 * Shows ad name with ad type badge, tier highlighting, and portfolio average context.
 */
export function DisplayLeaderboard({
  tieredCreatives,
  portfolioAvg,
  kpiType,
}: DisplayLeaderboardProps) {
  const formatKpi =
    kpiType === "cpa"
      ? formatCurrency
          ...
          <tbody>
            {creatives.map((creative) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-sm text-left">
                  <AdTypeBadge adType={String(creative.adType ?? "")} />
                </td>
