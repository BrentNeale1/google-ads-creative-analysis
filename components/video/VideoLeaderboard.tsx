...
import type { VideoPortfolioAvg } from "@/lib/queries/video";
import {
  formatNumber,
  formatPercentage,
  formatCurrency,
...
export interface VideoLeaderboardProps {
...
/** Get display name for a video creative */
function creativeName(creative: TieredCreative): string {
  const adName = String(creative.adName ?? "");
  if (adName) return adName;
  return `Video ${creative.adId}`;
}

/**
 * Top and bottom creative leaderboard tables for Video ads.
 * Shows ad name, standard metrics, plus video-specific columns:
 * View Rate, VTR (P100 completion rate), and CPV.
 */
export function VideoLeaderboard({
  tieredCreatives,
  portfolioAvg,
  kpiType,
}: VideoLeaderboardProps) {
  const formatKpi =
    kpiType === "cpa"
      ? formatCurrency
          ...
          <tbody>
            {creatives.map((creative) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-sm text-right">
                  {formatPercentage(Number(creative.videoViewRate) || 0)}
                </td>
                <td className="px-4 py-3 text-sm text-right">
                  {formatPercentage(Number(creative.videoQuartileP100Rate) || 0)}
                </td>
                <td className="px-4 py-3 text-sm text-right">
                  {formatCurrency(Number(creative.averageCpvAud) || 0)}
                </td>
              </tr>
            ))}
            ...
            <tr className="border-t-2 border-surface-gridline bg-[#F8F9FA]">
              ...
              <td className="px-4 py-3 text-sm text-right text-gray-600">
                {formatPercentage(portfolioAvg.avgVideoViewRate)}
              </td>
              <td className="px-4 py-3 text-sm text-right text-gray-600">
                --
              </td>
